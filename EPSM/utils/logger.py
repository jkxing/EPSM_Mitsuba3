...
class Loggers:
    ...
    def init(self, exp_name="experiment",path="/mnt/HDD/exp_log", show=False, debug=False, add_time=True):
        self.time = f'{datetime.datetime.now():%d-%b-%y-%H-%M-%S}'
        self.exp_name=exp_name
        self.prefix = path
        if add_time==False:
            self.path = os.path.join(self.prefix, self.exp_name)
        else:
    ...
    def save_param(self, name, arr):
        np.save(os.path.join(self.path,"params",name),arr)
    
    def save_img(self, name, img, flip=False):
    ...
    def save_img_2(self, name, img, flip=False):
        if torch.is_tensor(img):
            img = img.detach().cpu().numpy()
        img = img.astype(np.float32)
        img = img[...,:3]
        ...
        img = (np.clip(img,0,1)*255).astype(np.uint8)
        img = cv2.resize(img,(512,512),interpolation=cv2.INTER_CUBIC)
        cv2.imwrite(os.path.join(self.path,"images",name),img)
